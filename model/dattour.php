...
class DATTOUR{
	...
	public function themdattour($nguoidung_id,$diachi_id,$tongtien){
		$db = DATABASE::connect();
		try{
			$sql = "INSERT INTO donhang(nguoidung_id, diachi_id, ngay, tongtien, ghichu) 
					VALUES(:nguoidung_id,:diachi_id,:ngay,:tongtien,:ghichu)";
			$cmd = $db->prepare($sql);
	...
	public function docdattour($nguoidung_id ){
		$db = DATABASE::connect();
		try{
			$sql ="SELECT *, ct.id as idct FROM donhang dh, nguoidung nd, donhangct ct, diachi dc WHERE nd.id = dh.nguoidung_id and ct.donhang_id = dh.id and dc.id = dh.diachi_id and nd.email=:id and ct.xacnhan <> 1 ORDER BY dh.id";
			$cmd = $db->prepare($sql);
	...
	public function xacnhandattour($nguoidung_id,$id){
		$db = DATABASE::connect();
		try{
			var_dump($id);
